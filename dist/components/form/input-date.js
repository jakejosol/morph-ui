...
export default class InputDate extends React.Component {
	...
	checkDate() {
		...
		if (this.state.year !== '' && this.state.month !== '' && this.state.day !== '' && !date.isValid()) this.setState({ error: 'Invalid date' });else if (this.state.year && this.state.month && this.state.day) this.setState({ error: null }, () => {
			if (typeof this.props.onChange === 'function') this.props.onChange(date.format());
		});
	}
	...
	onYearChange(e) {
		...
		this.setState({ year: value.replace(/\D/g, '').substr(0, 4) }, () => {
			if (value.length >= 4) this.refs.month.focus();
			this.checkDate();
		});
	}
	...
	onMonthChange(e) {
		...
		this.setState({ month: value.replace(/\D/g, '').substr(0, 2) }, () => {
			if (value.length >= 2) this.refs.day.focus();
			if (value.length == 0) this.refs.year.focus();
			this.checkDate();
		});
	}
	...
	onDayChange(e) {
		...
		this.setState({ day: value.replace(/\D/g, '').substr(0, 2) }, () => {
			if (value.length == 0) this.refs.month.focus();
			this.checkDate();
		});
	}
